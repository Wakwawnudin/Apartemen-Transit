...
import { getBookings, saveBookings, calculateBufferEnd } from './bookingUtils';
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-8">
           <div className="bg-slate-800 rounded-[32px] p-6 border border-slate-700 shadow-2xl">
              ...
              <form onSubmit={handleAddBooking} className="space-y-4">
                 ...
                 <div>
                   ...
                   <input type="text" placeholder="Tamu Walk-in" value={guestName} onChange={(e) => setGuestName(e.target.value)} className="w-full bg-slate-900 border border-slate-700 rounded-xl p-3 text-sm font-bold text-white outline-none focus:border-[#D4AF37]" />
                 </div>
           ...
           <div className="bg-slate-800 rounded-[32px] p-6 border border-slate-700 shadow-2xl flex flex-col">
              <h2 className="text-sm font-black text-white uppercase tracking-widest mb-6 border-b border-slate-700 pb-4">Status Kalender Web</h2>
              <div className="flex-1 overflow-y-auto space-y-3 no-scrollbar pr-2">
                 {filteredBookings.length === 0 ? (
                    ...
                    filteredBookings.map(b => {
                       ...
                       return (
                         <div key={b.id} className="bg-slate-900 border border-slate-700 rounded-2xl p-4 flex justify-between items-center group hover:border-[#D4AF37]">
                            <div>
                            ...
                            <button onClick={() => handleRemoveBooking(b.id)} className="bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white p-3 rounded-xl">
                               <Trash2 size={16} />
                            </button>
                         </div>
                       )
